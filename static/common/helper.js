...
function UpdateScoreboard(state, htmlScore) {
	...
	for (const id in state.players)
		list.push([id, state.players[id].score]);
	list.sort((a, b) => ((a[1] < b[1] || (a[1] == b[1] && a[0] > b[0])) ? 1 : -1));

	/* add the list of players */
	for (let i = 0; i < list.length; ++i) {
		const player = state.players[list[i][0]];
		...
		/* define the line writer to write lines to the scoreboard */
		let node = htmlScore.children[i], count = 0;
		const makeNext = function () {
		...
		makeNext().innerText = `Name: ${player.name}${player.ready ? ' (Ready)' : ''}`;
